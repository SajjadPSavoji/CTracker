...
def main(args=None):
	...
	parser.add_argument('--log_path', type=str, default="./train_log.txt" )
	parser.add_argument('--from_pretrained', type=str, default="false")
	...
	dataloader_train = DataLoader(dataset_train, num_workers=25, collate_fn=collater, batch_sampler=sampler)

	# Create the model
	if parser.from_pretrained == "true":
	...
	if parser.from_pretrained == "false":
	...
	if parser.from_pretrained == "true":
		scheduler = torch.load(os.path.join(parser.model_dir, 'scheduler.pt'))
	...
	for epoch_num in range(parser.epochs):
		...
		for iter_num, data in enumerate(dataloader_train):
			try:
				...
				loss = (classification_loss + regression_loss + reid_loss)/3
				...
				with open(parser.log_path, "a") as f:
					print('Epoch: {} | Iter: {} | Cls loss: {:1.5f} | Reid loss: {:1.5f} | Reg loss: {:1.5f} | Running loss: {:1.5f}'.format(epoch_num, iter_num, float(classification_loss), float(reid_loss), float(regression_loss), np.mean(loss_hist)), file=f)
		...
		torch.save(scheduler, os.path.join(parser.model_dir, 'scheduler.pt'))
