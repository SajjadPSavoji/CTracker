...
class ResNet(nn.Module):
    ...
    def forward(self, inputs, last_feat=None):
        ...
        if self.training:
            ...
            track_features = []
            for ind, featmap in enumerate(features):
                track_features.append(torch.cat((last_feat[ind], featmap), dim = 1))
            ...
            final_bboxes = cython_soft_nms_wrapper(0.7, method='gaussian')(torch.cat([transformed_anchors[:, :, :].contiguous(), scores, reid_score], dim=2)[0, :, :].cpu().numpy())
